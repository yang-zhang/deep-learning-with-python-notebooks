...
def copy_sample(dir_source, dir_destin, file_type, n):
    ...
    fs_shuffle = np.random.permutation(fs)
    for i in range(n):
        shutil.copyfile(dir_source + '/' + fs_shuffle[i],
                        dir_destin + '/' + fs_shuffle[i])


def get_batches(dirname, gen=image.ImageDataGenerator(),
                shuffle=True, batch_size=4, class_mode='categorical',
                target_size=(224, 224)):
...
def plot_keras_model(model, show_shapes=True, show_layer_names=True):
    return SVG(model_to_dot(model,
                            ...
                            show_layer_names=show_layer_names).create(
                            prog='dot', format='svg'))
